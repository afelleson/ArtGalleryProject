...
void commentDB::addComment(string name_input, string text_input, string artworkID, string x_input,string y_input, string width_input, string rating_input,string isPinned_input){
	...
	if (!conn) {
  	...
  	stmnt->executeQuery("INSERT INTO Comments(Name, CommentText, ArtworkID, SelectionXCoord, SelectionYCoord, SelectionWidth, Rating, isPinned) VALUES ('"+name_input+"','"+artworkID+"','"+x_input+"','"+y_input+"','"+width_input+"','"+rating_input+"','"+isPinned_input+"')");
}
